...
class MatterRelayPlatform {
    ...
    async start() {
        ...
        for (const { platform, relay, uuid } of targets) {
            ...
            const binding = {
                owner: this, ready: false, pending: Promise.resolve(),
                sync: power => {
                    if (!binding.ready || this.stopped) return;
                    const onOff = Boolean(power);
                    binding.pending = binding.pending.then(() => {
                        if (binding.ready && !this.stopped) return matter.updateAccessoryState(uuid, 'onOff', { onOff });
            ...
            const command = on => {
                ...
                setTimeout(() => platform.controller.getRelayState(relay.address), 400);
            };
            const accessory = {
                UUID: uuid, displayName: relay.name,
                deviceType: matter.deviceTypes.OnOffLight,
                manufacturer: 'BTicino / Legrand', model: 'MyHome MHRelay',
                serialNumber: uuid.replace(/-/g, ''),
                clusters: { onOff: { onOff: Boolean(relay.power) } },
                handlers: { onOff: {
                    on: async () => command(true), off: async () => command(false),
