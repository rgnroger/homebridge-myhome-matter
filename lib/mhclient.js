    ...
    onMonitor(_frame) {
        ...
        frames.forEach(function (frame) {
            ...
            if (extract) {
                ...
                if (this.parent != null && this.parent.onMonitor != null && typeof (this.parent.onAdvancedBlind) == 'function') this.parent.onAdvancedBlind(address, direction, position);
            }
            ...
            // DRYCONTACT: 	*25*31#[0-1]*3<where>##   -> Open   -> TRUE
			// 				*25*32#[0-1]*3<where>##   -> Closed -> FALSE
            // CEN+ 4680: commands 21-24 are short, start long, repeat and release.
            extract = frame.match(/^\*25\*2([1-4])#(\d+)\*2(\d+)$/);
            if (extract) {
                const eventNames = {
                    1: 'SHORT_PRESS',
                ...
                let event = eventNames[parseInt(extract[1], 10)];
                let button = parseInt(extract[2], 10);
                let address = parseInt(extract[3], 10);
                if (this.parent != null && this.parent.onMonitor != null && typeof (this.parent.onCenPlus) == 'function')
                    this.parent.onCenPlus(address, button, event);
                debug(sprintf("CEN+ %s BUTTON %s ADDRESS %s", event, button, address));
            }
...
class MyHomeConnection {
    constructor(_ipaddress, _port, _password, _type, _onconnectcallback, _onmessagecallback, _setclock) {
        ...
        if (this.type == "COMMAND") {
            this.keepalivehandle = setInterval(function () { this.sendKeepalive(); }.bind(this), this.keepaliveSeconds * 1000);
            if (this.setclock)
                this.updateTimehandle = setInterval(function () { this.updateDate(); }.bind(this), this.updateTimeSeconds * 1000);
        }
    ...
    connect() {
        ...
        this.connection.on('data', (_in) => {
            ...
            var terminator;
            while ((terminator = this.receiveBuffer.indexOf("##")) >= 0) {
                var data = this.receiveBuffer.substring(0, terminator + 2).trim();
                this.receiveBuffer = this.receiveBuffer.substring(terminator + 2);
                if (data != "") this.processFrame(data);
            }
        });
    ...
    stop() {
        ...
        if (this.disconnecthandle != null) clearTimeout(this.disconnecthandle);
        if (this.keepalivehandle != null) clearInterval(this.keepalivehandle);
        if (this.updateTimehandle != null) clearInterval(this.updateTimehandle);
        this.timerhandle = null;
        this.reconnecthandle = null;
        this.disconnecthandle = null;
        this.keepalivehandle = null;
     ...
     */
    write(_in) {
        if (this.connection == null || this.connection.destroyed || !this.connection.writable) return false;
