...
function sendEvent(service, characteristic, value) {
  if (!service || characteristic === undefined) return;
  const target = service.getCharacteristic(characteristic);
  if (typeof target.sendEventNotification === 'function') {
    target.sendEventNotification(value);
  } else {
    target.updateValue(value);
  }
}

function contactSensorStateFromDryContact(open, invert) {
  let state = Boolean(open);
  if (invert) state = !state;
  return state;
}

function cenPlusEventToHomeKit(event, Characteristic) {
  if (event === 'SHORT_PRESS') {
    return Characteristic.ProgrammableSwitchEvent.SINGLE_PRESS;
  }
  if (event === 'RELEASE_EXTENDED_PRESS') {
